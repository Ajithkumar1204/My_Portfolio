...
const projects = [
  {
    ...
    desc: 'Enterprise-grade Employee Management System with JWT Authentication, Role-Based Access Control, Dashboard Analytics, Employee & Department Management, Profile Image Upload, PDF/Excel Export, and Cloud Deployment.',
    stack: [
      'React',
      'Java',
      'Spring Boot',
  ...
  return (
    <section id="projects">
      <div className="container">
        ...
        {/* ── Desktop: carousel ─────────────────────────────────── */}
        {!isMobile && (
          <div className="fade-in projects-carousel" style={{ position: 'relative', padding: '0 54px' }}>
            ...
            <button onClick={prev} disabled={current === 0} style={{
              ...
              transition: 'all 0.2s', zIndex: 10, outline: 'none', border: 'none',
            }}>‹</button>
            ...
            <div style={{
              ...
              {projects.map((p, i) => {
                ...
                return (
                  <div key={p.id} onClick={() => !isCenter && setCurrent(i)} style={{
                    flex: isCenter ? '0 0 65%' : '0 0 17%',
                    minHeight: isCenter ? '360px' : '280px',
                    borderRadius: '20px',
                    background: isCenter ? 'rgba(255,255,255,0.75)' : 'rgba(255,255,255,0.45)',
                    border: isCenter ? '1.5px solid rgba(255,255,255,0.95)' : '1.5px solid rgba(255,255,255,0.7)',
                    backdropFilter: 'blur(16px)',
                    boxShadow: isCenter ? '0 20px 60px rgba(91,60,245,0.15)' : '0 8px 24px rgba(91,60,245,0.07)',
                    ...
                    {isCenter && p.type === 'project' && (
                      <>
                        ...
                        <div style={{ flex: 1, padding: '32px 28px', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '14px' }}>
                          ...
                          <h3 style={{ fontFamily: "'Outfit',sans-serif", fontSize: '22px', fontWeight: 800, color: '#1a1340', lineHeight: 1.2 }}>{p.title}</h3>
                          <p style={{ fontSize: '13px', color: '#5a5585', lineHeight: 1.7 }}>{p.desc}</p>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                            <div style={{ fontSize: '12px', fontWeight: 700, color: '#1a1340' }}>Tech Stack</div>
                            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                              {p.stack.map(s => <StackBadge key={s} name={s} />)}
                            </div>
                          </div>
                          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginTop: '6px' }}>
                            <a href={p.demo} target="_blank" rel="noreferrer" style={{
                              ...
                              boxShadow: '0 6px 20px rgba(91,60,245,0.35)', transition: 'all 0.2s',
                            }}
                            ...
                            onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(91,60,245,0.2)'; e.currentTarget.style.color = '#1a1340'; }}>
                              ⬡ GitHub
                            </a>
                          </div>
                        </div>
                    ...
                    {!isCenter && (
                      <div style={{ padding: '20px', textAlign: 'center' }}>
                        ...
                        <div style={{ fontSize: '15px', fontWeight: 700, color: '#4a4580' }}>{p.title}</div>
                      </div>
                    )}
                  </div>
                );
      ...
      <style>{`
        ...
        @media (max-width:480px) {
          .proj-stats-grid { grid-template-columns: 1fr !important; }
        }
      `}</style>
    </section>
  );
...
function MobileCard({ p }) {
  if (p.type === 'coming-soon') {
    return (
      <div style={{
        borderRadius: '20px', background: 'rgba(255,255,255,0.75)',
        border: '1.5px solid rgba(255,255,255,0.95)', backdropFilter: 'blur(16px)',
        boxShadow: '0 12px 40px rgba(91,60,245,0.12)',
        padding: '36px 24px', textAlign: 'center',
        animation: 'fadeSlideUp 0.5s ease both',
      }}>
        <div style={{ fontSize: '48px', marginBottom: '12px', animation: 'iconFloat 3s ease-in-out infinite' }}>{p.emoji}</div>
        <div style={{ fontSize: '11px', fontWeight: 700, color: '#5b3cf5', letterSpacing: '1.5px', marginBottom: '8px' }}>{p.num}</div>
        <h3 style={{ fontFamily: "'Outfit',sans-serif", fontSize: '20px', fontWeight: 800, color: '#1a1340' }}>{p.title}</h3>
        <p style={{ fontSize: '14px', color: '#8b87b8', marginTop: '8px' }}>{p.desc}</p>
      </div>
    );
  }
  ...
  return (
    <div style={{
      borderRadius: '20px', background: 'rgba(255,255,255,0.78)',
      border: '1.5px solid rgba(255,255,255,0.95)', backdropFilter: 'blur(16px)',
      boxShadow: '0 12px 40px rgba(91,60,245,0.13)',
      overflow: 'hidden', animation: 'fadeSlideUp 0.5s ease both',
    }}>
      {/* Coloured top bar */}
      <div style={{
        background: 'linear-gradient(135deg, rgba(91,60,245,0.12) 0%, rgba(123,97,255,0.08) 100%)',
        borderBottom: '1px solid rgba(255,255,255,0.7)',
        padding: '20px 20px 16px',
        display: 'flex', alignItems: 'center', justifyContent: 'space-between',
      }}>
        <div>
          <div style={{ fontSize: '11px', fontWeight: 700, color: '#5b3cf5', letterSpacing: '1.5px', marginBottom: '4px' }}>{p.num}</div>
          <h3 style={{ fontFamily: "'Outfit',sans-serif", fontSize: '17px', fontWeight: 800, color: '#1a1340', lineHeight: 1.25, margin: 0 }}>{p.title}</h3>
        </div>
        <div style={{
          width: '44px', height: '44px', borderRadius: '12px', flexShrink: 0,
          background: 'linear-gradient(135deg,#5b3cf5,#7b61ff)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: '22px', boxShadow: '0 4px 14px rgba(91,60,245,0.35)',
        }}>
          {p.emoji || '💡'}
        </div>
      </div>
      ...
      <div style={{ padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
        <p style={{ fontSize: '13px', color: '#5a5585', lineHeight: 1.7, margin: 0 }}>{p.desc}</p>

        {/* Stack */}
        {p.stack.length > 0 && (
          <div>
            <div style={{ fontSize: '11px', fontWeight: 700, color: '#1a1340', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.8px' }}>Tech Stack</div>
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
              {p.stack.map(s => <StackBadge key={s} name={s} small />)}
            </div>
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          {p.github && p.github !== '#' && (
            <a href={p.github} target="_blank" rel="noreferrer" style={{
              flex: 1, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '6px',
              padding: '12px 0', borderRadius: '50px',
              background: 'rgba(255,255,255,0.95)', color: '#1a1340',
              border: '1.5px solid rgba(91,60,245,0.22)',
              textDecoration: 'none', fontSize: '13px', fontWeight: 700,
  ...
  if (!p.preview) {
    return (
      <div style={{ textAlign: 'center' }}>
        <div style={{ fontSize: '72px', animation: 'iconFloat 3s ease-in-out infinite', marginBottom: '12px' }}>{p.emoji}</div>
        <div style={{ fontSize: '13px', color: '#8b87b8', fontWeight: 600 }}>Live Project</div>
      </div>
    );
  }
  return (
...
/* ── Stack badge ─────────────────────────────────────────── */
function StackBadge({ name, small }) {
  const icons = {
    React: '⚛️',
    Java: '☕',
    'Spring Boot': '🌿',
  ...
  return (
    <div style={{
      ...
      fontWeight: 700, color: '#5b3cf5', whiteSpace: 'nowrap',
    }}>
